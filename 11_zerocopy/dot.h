...
template <std::size_t numBlocks = 1024, std::size_t numThreadsPerBlock = 256>
void dot_gpu_zerocopy(double* x_host, double* y_host, double& r,
                      std::size_t n) {
    double* x_device{nullptr};
    double* y_device{nullptr};

    auto status = cudaHostGetDevicePointer(&x_device, x_host, 0);
    cuda_check_status(status);
    status = cudaHostGetDevicePointer(&y_device, y_host, 0);
    cuda_check_status(status);

    // Results of partial sums computed with numBlocks
    double* r_host{nullptr};
    double* r_device{nullptr};
    r_host = malloc_mapped<double>(numBlocks);
    status = cudaHostGetDevicePointer(&r_device, r_host, 0);
    cuda_check_status(status);

    dot_kernel<numThreadsPerBlock>
        <<<numBlocks, numThreadsPerBlock>>>(x_device, y_device, r_device, n);

    // Syncronyse CPU and GPU
