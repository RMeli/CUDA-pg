...
T* malloc_device(std::size_t n){
    ...
    auto status = cudaMalloc(&device_ptr, n * sizeof(T)); // Try memory allocation
...
void copy_host_to_device(T* host_ptr, T* device_ptr, std::size_t n){
    auto status = cudaMemcpy(device_ptr, host_ptr, n * sizeof(T), cudaMemcpyHostToDevice);
...
void copy_device_to_host(T* device_ptr, T* host_ptr, std::size_t n){
    auto status = cudaMemcpy(host_ptr, device_ptr, n * sizeof(T), cudaMemcpyDeviceToHost);
    cuda_check_status(status);
}
